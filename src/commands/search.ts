...
async function printOrtDiagnostics(): Promise<void> {
  try {
    const { ortRuntimeVersion } = await import('../lib/embedding')
    console.error(`诊断: onnxruntime 实际加载版本 = ${await ortRuntimeVersion()}（期望 1.21）`)
...
export async function cmdSearch(args: string[], options: CommandOptions): Promise<void> {
  ...
  // reindex（增量对账）与检索（查询词 embed）都要过 ORT——统一纳入错误处理
  try {
    ...
    console.error(`检索失败: ${String(e)}`)
    await printOrtDiagnostics()
    process.exitCode = 1
  }
}
