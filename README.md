...
- **小月管家模式**（桌面端小月面板默认开启「⚙ 工具」；CLI 用 `xiaoyue ... --tools`）：小月不止能答——通过 MoonLink 远程 MCP 调用全部 29 个工具帮你**动手**：收藏网页、记便签待办、保存记忆、查询书房。写操作执行前需确认（桌面端弹确认条，CLI 终端 y/N；只读查询自动执行），每步操作可见（`⚙` 活动流）；
...
- 下行为增量游标分页，每 20 篇落一次断点（中断续跑不整体重来）；收集箱上行每篇即时落账（中断不重复上传）；
