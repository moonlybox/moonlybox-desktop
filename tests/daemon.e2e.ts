...
// #256：settings 通道——get 全量+提供商清单 / save 分节合并 / 空 patch 拒绝 / 平台API key 不落 settings.json
const stGet0 = await rpc('settings', { op: 'get' }, 15_000)
const stGet0Ok = stGet0.at(-1)?.event === 'done' && (() => { try { const d = JSON.parse(String(stGet0.at(-1)?.text ?? '{}')); return d.ok === true && d.settings && typeof d.settings.appearance?.theme === 'string' && Array.isArray(d.providers?.platform) && d.providers.platform.length >= 5 } catch { return false } })()
assert('settings get 全量+提供商清单', stGet0Ok, JSON.stringify(stGet0.at(-1))?.slice(0, 120))
const stSave1 = await rpc('settings', { op: 'save', patch: { appearance: { theme: 'dark', zoom: 125 }, chat: { maxRetries: 5 } } }, 15_000)
const stSave1Ok = stSave1.at(-1)?.event === 'done' && (() => { try { const d = JSON.parse(String(stSave1.at(-1)?.text ?? '{}')); return d.settings.appearance.theme === 'dark' && d.settings.appearance.zoom === 125 && d.settings.chat.maxRetries === 5 && d.settings.chat.contextEnabled === true } catch { return false } })()
assert('settings save 分节合并（未动字段保默认）', stSave1Ok)
const stSaveEmpty = await rpc('settings', { op: 'save', patch: {} }, 4000)
assert('settings save 空 patch 拒绝', stSaveEmpty.at(-1)?.event === 'done' && stSaveEmpty.at(-1)?.code === 1)
const stSaveByok = await rpc('settings', { op: 'save', patch: { model: { provider: 'deepseek', baseUrl: 'https://api.deepseek.com/v1', model: 'deepseek-chat', apiKey: 'sk-test-1234567890' } } }, 15_000)
const stSaveByokOk = stSaveByok.at(-1)?.event === 'done' && (() => { try { const d = JSON.parse(String(stSaveByok.at(-1)?.text ?? '{}')); return d.settings.model.provider === 'deepseek' && !JSON.stringify(d.settings.model).includes('sk-test') } catch { return false } })()
assert('settings 平台API 保存（key 不落 settings.json）', stSaveByokOk)
const stSaveBad = await rpc('settings', { op: 'save', patch: { model: { provider: 'x', baseUrl: '', model: '', apiKey: 'sk-abc12345' } } }, 4000)
assert('settings 平台API 缺地址/模型名拒绝', stSaveBad.at(-1)?.event === 'done' && stSaveBad.at(-1)?.code === 1)
const stBad = await rpc('settings', { op: 'nope' }, 4000)
assert('settings 未知 op 拒绝', stBad.at(-1)?.event === 'done' && stBad.at(-1)?.code === 2)
